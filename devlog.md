...
**`design/14.0` v0.14 分区 K（A1–A5）：**
- A1 数据绝对中心：state + JSON 是唯一真相，逻辑永不绑渲染树。
- A2 面板走现代 UI：目标 Vue，组件合同「上文下钮」。
- A3 网格用轻量 2D：目标 Pixi，网格立项再上。
- A4 分期四步，禁止全量重写，开局 30 分钟不得停摆。
- A5 本品类走 Web 数据驱动；物理/3D/多端一键不作为本栈目标。

定位表技术栈/平台路线对齐。`CLAUDE.md` 项目简介同步。

---

## 2026-08-23 — 机缘空窗 / 点了没内容

用户实机：机缘抽屉「功德与劫气·再择」点查看机缘后只剩一条空金框；同类机缘反复如此。

**根因：**
1. 抽屉只写「尚未抉择」再跳第二层弹窗；`currentPopup` 若残留（层已关），`drainPopupQueue` 直接 return，弹窗不出来。
2. 弹窗骨架（title/body/buttons）若被别的 renderer 拆掉，再开会画出空匾。
3. `renderEventPopup` 缺行时 `closePopup()` 重入，闪一下空框。

**改：**
- 机缘抽屉直接展开正文+选项，不再跳「查看机缘」。
- 主钮「天象有变」先清残留锁再弹。
- 弹窗骨架每次重建；渲染 try/catch；空机缘给「已散」而不是空框。
- 选项用 DOM 不拼 innerHTML；减资源显示「劫气 -80」。
- `style.css?v=20260823e`。

**验收（满分 100，≥95 过）**

| 维 | 权 | 分 | 核对 |
| 抽屉即抉择 | 30 | 30 | 314 卷内见散劫气/炼功德，无空跳 |
| 主钮能弹 | 25 | 25 | 人为 stale lock 后仍弹出全文+两钮 |
| 不空框 | 20 | 20 | 弹窗高 343，无空 label 钮 |
| 记下/同类 | 15 | 15 | 101 记下；217 功德压劫/劫气噬德 |
| 结算 | 10 | 10 | 选后 pending 清空，收下机缘有账 |

**100 / 100**。`实盘测试/event-popup-2026-08-23/` EVENT UI PASS。

---

## 2026-08-23 — 机缘保底稀有度 / 四修术法 / 天仙神通

用户：新设计机缘、术法、神通没落码。按完整范围四件事一次落地（保底、稀有度、四修下线旧表、真仙觉醒）。

**1 机缘稀有度（design/1.4 §4 + 表 rarity_config）**
抽签先掷档（普通 70 / 精良 25 / 稀有 4.5 / 天命 0.5）再在该档加权。缺 rarity 的 78 条已补齐。弹窗标题「机缘·普通：山中异象」。

**2 机缘保底（1.4 §5 + 表 pity_rule）**
- 每日首次闭关必出普通或精良
- 连续两日无稀有/天命，第三日首次升稀有
- 第 3–5 天保底 `event_006` 阐教清光（跟脚/清光）
- 本命法宝择主后保底 `event_201` 法宝初鸣
state.event_pity 日切。机缘页写明档位与保底。

**3 术法面板改四修，下线 spell_table**
抽屉按体/器/魂/劫。雷归体；器区做法宝+五行，御器/火术标明是法术不是器。旧 23 门参悟列表不再渲染。遭遇检定、战力、spell_level 改走 skill_table。

**4 天仙术法觉醒成神通**
SkillIdentity：炼气本名、真人/地仙 advanced_names、天仙 shentong_name 且 class=神通。升入天仙弹「术法觉醒·神通」；斗法栏名与「通·」格用觉醒名。先天神通仍随化身，不在此列。

**验收（满分 100，≥95 过）**

| 维 | 权 | 分 | 核对 |
| 稀有度四档抽 | 20 | 20 | 表四档皆有；首次收菜 400 次仅普通+精良 |
| 四条保底 | 20 | 20 | day3→event_006；days_without_rare=2→epic/fate；法宝择主 offer 201 |
| 四修面板+旧表下线 | 25 | 25 | 【体/器/魂/劫】；无【雷系】/五道择一/参悟首门；真人见金刚体；器是法宝 |
| 神通觉醒 | 25 | 25 | tx_01 肉身成圣·神通；炼气仍炼体拳·法术 |
| 机缘 UI | 10 | 10 | 标题「机缘·普通：山中异象」；页内保底文案 |

**100 / 100**。`node scripts/design_four_smoke.js` PASS；Playwright DESIGN FOUR UI + EVENT UI PASS。

---

## 2026-08-23 — 主页拿掉卷一卷轴、立绘不再被资源条挡脸

用户实机：红框「卷一·山野炼气」可不上主页；资源条（后期法器/法宝碎片）把脸挡住，沉浸差。根因是 `#stage` 绝对铺满整屏，立绘画在筹码底下。

**改：**
- `#goal-panel` 主页 `display:none`（卷 / 洞府手札仍可看）。
- 舞台改文档流 `flex:1`，夹在资源条与道行槽之间，人落在空档里。
- 5–8 枚筹码走 3 列网格，名字不截断。
- `style.css?v=20260823d`。

**验收（满分 100，≥95 过）**

| 维 | 权 | 分 | 核对 |
| 主页无卷一 | 30 | 30 | `#goal-panel` display:none；卷仍开得开 |
| 筹码不挡脸 | 35 | 35 | 开局 2 筹 / 后期 8 筹，舞台 top ≥ 条 bottom |
| 人在空档 | 20 | 20 | 390×844 立绘高 389（开局）/ 326（8 筹） |
| 短屏不叠 | 15 | 15 | 390×667 重叠面积 0 |

**100 / 100**。截图 `实盘测试/home-layout-2026-08-23/`。HOME LAYOUT PASS。

---

## 2026-08-23 — 继续闭关弹出说明框、不再打坐

用户：主钮「继续闭关」点下去是「静心闭关」说明，不是吐纳/入定。根因是吐纳日限 40、入定 20；连续修行用尽后主钮掉进 idle，点击走离线说明弹窗。

**改：** 吐纳/入定 `daily_limit=0`（洞府主循环不限次）。选取/续作跳过这两条的日限。点 idle 也先 `startPreferredCultivation()`，有坐可坐才弹说明。

**验收：** 日计数 99 时主钮仍「入定一轮」；把按钮强制成 idle 再点，current=short_meditation，无「闭关」弹窗。SIT LOOP PASS。

---

## 2026-08-23 — 六抽屉卡片被按钮挤碎

用户游历卷：地名/正文被挤成碎片，金钮贴在右侧像乱码。根因是加厚 9-slice 边后仍横排卡片，`card-info min-width:0` + `overflow-wrap:anywhere`，长按钮把标题压成一字一行。

**改：** 卡片改 grid：上排图+文，下排动作钮靠右；禁 anywhere 断字；游历多钮用 `card-btn-col`。境界/游历/术法/法宝/机缘/洞府都过了一遍。`style.css?v=20260823c`。

**验收（满分 100，≥95 过）**

| 维 | 权 | 分 | 核对 |
| 游历正文可读 | 30 | 30 | 山野妖患 / 陈塘关外围整行可读，无一字一列 |
| 按钮不挤文 | 25 | 25 | 驻留+游历同一行靠右；斗法/升重/温养 justify-self end，rightGap 46 未出云纹 |
| 六抽屉过一遍 | 25 | 25 | 境界/游历/术法/法宝/机缘/洞府 390×844 截图 |
| 卷内不压框 | 20 | 20 | card 宽 298，未溢出 sheet 左右 36px |

**100 / 100**。截图 `实盘测试/panel-cards-2026-08-23/`。

---

## 2026-08-23 — 主按钮不再跑陈塘巡行

用户截图：洞府主页主钮写着「陈塘巡行中」，连续修行开着。设计已接入过：主页挂机只走洞府修行，游历/巡行是并列选择，走游历页。

**根因：** 自动续作已经不跟 map 动作，但 `_preferredCultivationAction` 仍把 `wild_travel / chentang_patrol / kulou_explore` 排进主钮。吐纳 40 次、入定 20 次用尽后，主钮和挂机都会掉进巡行。续作失败时 `|| currentRow` 还会把正在进行的巡行接着刷。

**改：**
- 主钮只选入定/吐纳/师门功课。
- 日次数用尽 →「继续闭关」，不改去游历。
- 连续修行续作若当前是巡行，改回吐纳；不再 `|| currentRow`。
- 游历页仍可手动点陈塘巡行；进行中那一段会跑完，下一段回修行。

**验收（满分 100，≥95 过）**

| 维 | 权 | 分 | 核对 |
| 主钮只修行 | 40 | 40 | zr_07 idle：入定一轮，DOM 同文 |
| 挂机不续巡行 | 25 | 25 | 巡行结束 tick → current=breath_cycle |
| 游历仍能巡行 | 15 | 15 | chentang_patrol availability.ok |
| 次数用尽 | 10 | 10 | 吐纳入定日满 → type=idle 继续闭关 |
| 进行中不掐 | 10 | 10 | current=chentang 时主钮仍「陈塘巡行中…」 |

**100 / 100**。Playwright MAIN CULTIVATION PASS。

---

## 2026-08-23 — 新机缘 78 条接入运行时

用户：日志里新机缘怎么没接到游戏里。表有 98 行（旧 20 + event_101 起 78），运行时几乎只出旧 20。

**根因（不是缺表，是触发链）：**
1. 新机缘 `weight` 写成 0.02–0.08 出现率，旧机缘 100+ 彩票。混抽永远是旧的。
2. 吐纳 `event_chance` 曾是 0；`event_system` 锁在炼气六重，前 30 分钟抽签根本不开。
3. ContentDirector 以前只认 `minigame_id`，节拍写了 `event_id` 也会被跳过；有 pending 时还会把节拍标成已放，榜文碎光被烧掉。
4. `weight=0` / 缺 weight 是脚本机缘（探索点、入局、首通），却被当成抽签项或直接 `canOffer` 拒绝。
5. `array` / `faction` 从来没人 `rollEvent`；`chooseFaction`、破阵、丹房、真灵上榜都不挂机缘。
6. Boss 首通误绑 event_401–403，和山野探索点抢同一条 lifetime=1。

**改：**
- 抽签：0~1 乘 1000 与旧彩票同池；缺省/0 不进抽签；`canOffer` 不看 weight（探索点/首通能弹）。
- 日上限走表 `daily_limit_rule`（1 日 3 / 2 日 4 / 之后 5）。
- 无 weight 的 offline 脚本机缘：登录/升重每次一条。
- 钩子：入局 316–319、杀阵 320+321–324、丹房 325、真灵 328–330、探索点 `_offerEvent`。
- 开局：炼气二重开系统、吐纳 3%、节拍 `seal_light_d0`→event_001；升重不再叠「新机缘开启」功能弹。
- Boss 008–010 首通改为 321–323，不再偷 401–403。

**验收（满分 100，≥95 过）**

| 维 | 权 | 分 | 核对 |
| 抽签标尺 | 20 | 20 | rq_02 离线 400 抽：001/101/102 同池，新机缘 >80 次。页内 200 抽 newHits=104 |
| 开局门 | 20 | 20 | event_system=rq_02；吐纳 0.03；节拍绑 001；pending 不烧节拍 |
| 脚本机缘 | 20 | 20 | 401 canOffer 真、lottery 0；316 不进 offline；nextScripted 不吐入局 |
| 系统钩子 | 25 | 20 | 势力/杀阵/丹房/真灵/探索已挂。event_404/405 表内仍无行，Boss011/012 首通空转 |
| 空选项收束 | 10 | 10 | Playwright：山中异象「记下道行 +30」，pending 清 |
| 弹窗克制 | 5 | 5 | 日上限 3/4/5；升重跳过 event_system 功能弹 |

**95 / 100**，过线。扣在金仙后两只 Boss 的 404/405 缺行。

证据：`node scripts/event_wire_smoke.js` ALL PASS；页内 WIRE E2E PASS；`tests/event-stuck.cjs` PASS。

---

## 2026-08-23 — 抽屉文字冲出卷轴

用户六张图：境界/游历/术法/法宝/机缘/洞府，字压到云纹外。根因：侧边 9-slice 只有 18px，flex 子项 `min-width:auto`，斗法栏卡片把标题/正文/按钮横排。

改：侧边收到 44px、slice 110；body/card `min-width:0` + 断词；斗法栏改 card-info 竖排；纸上的次按钮不用玉笺。`style.css?v=20260823b`。

---


## 2026-08-23 — 战斗以共鸣扩展§〇为准（体系四修 + 器二级五行）

用户指定压过 8.0/14.0「6 系不动」。

**口径：** 体系=体/器/魂/劫（无互克）；五行=木火土金水，只做器的二级；器=法宝、无招式；方案 B 每格绑法宝。雷归体。御器/火意象招是法术不是器。

**改：** 14.0 战斗体系行改锁。配招四修：体法术（含原雷）、器只做法宝绑格+五行标签、魂/劫法术。连锁按体系（体含 thunder）。栏外空「宝/通」不再当体系。

---


## 2026-08-22 — 斗法栏回到六格 + 四修二级（design/8.0 / 11.0）

用户两问：① 人物太大；② 法宝/神通/术法不是栏外「宝/通」空槽，而是放进斗法栏；③ 身份轴是体/器/魂/劫四修再分二级，不是六系平铺。

**设计核对：** 8.0 三品类共享斗法栏、炼气 3 格上限 6 格；11.0 四修为主轴，雷火挂器修·五行，引擎 6 系连锁不动。

**改：**
- 主屏立绘在 `body.dueling` 时隐藏；己方圆像缩到 64px。
- 斗法栏固定六格（炼气 3 开 + 3 锁），格上标 术/宝/通；拆掉栏外宝/通芯片。法宝可作格内技能放入。
- 配招按四修分组：主系 + 二级（器·五行先见雷火；玄功/金身/出窍等金仙后开）。

lifecycle PASS：6 格、四修 4 组、无栏外宝槽。

---


## 2026-08-22 — 斗法场回到榜卷骨架（用户参考图）

用户：现在战斗还不如之前「敌在金卷、洞府当世界、底三术印」那一版。对站把洞府挤没了。

**改：** `#duel-root.duel-board` 按参考图重排——榜卷大敌、草上圆像、出手行「体 炼体拳 伤 xx」、底三术印。敌招三格贴在卷上（扑击/嘶吼震魂可读），催动仍点亮印，宝/通仍在圆像旁。去掉贯穿山水的白线。会话隔离、跳过、收取不改。

**验收：** lifecycle PASS。8.5 斗法复评 97（B1 口径按用户终评更正：天卷是榜文锚点）。截图 `实盘测试/battle-board-2026-08-22/`。

---


## 2026-08-22 — 9.6 打到 128（过线 120）

用户两图判 30。改：玉牌导航（二字，锁定同玉半透明）、金匾行动、玉筹码、褐墨声音设置、粗体 Noto/隶书。`style.css?v=20260823a`。

实评范围 86 + 超额 42 = **128**。刚过。主按钮仍是金条、复选框偏原生，不报满分。

---


## 2026-08-22 — 9.6 改计分：满分 100、过线 120；现机 30

用户：满分 100 是做完，≥120 才通过（远超预期）。底栏玉璧糊、字太细、声音设置浅金字看不清。**现评 30，废弃。**

改：文楷 TC 错字（录→条）换 Noto Serif SC 700 + 系统隶书；底栏改为金笺玉牌二字，不再缩玉璧照片；设置标签改褐墨，滑条自制。`style.css?v=20260822w`。
...
## 2026-08-22 — 全界面高级感（9.6）与「尚未开启」黑窗

**现象**：随手点锁定术法，弹出旧黑底圆角 + 金胶囊，和卷轴主屏两款游戏。

**先评分再落**：立 `design/9.6`（满分 150，≥120 过）。方案：器物层保留切图，文书层 CSS 金线宣纸匾额，不新堆 PNG。方案自评 136。

**落地**：`#popup-panel` 改宣纸匾额（2px 折角、金线、褐墨）；`.popup-btn` 改金笺。`style.css?v=20260822u` 破缓存。D4u 入 14.0。

**实评**：尚未开启计算样式为宣纸渐变 + 2px 金笺。总分 **131 / 150**，过 120。截图 `hud-skin/05-popup-locked.png`。

---


## 2026-08-22 — 字体与 Q 版金玉卷轴不对味

**根因**：全局 `Songti SC`。宋体是刻本公文，立绘/卷轴是 Q 版手绘。iPhone 上宋体不存在，还会掉进苹方，两头都不像封神。

**改**：
- 正文 `LXGW WenKai TC`（霞鹜文楷）+ 系统楷体回退。
- 题写/按钮/底栏 `ZCOOL XiaoWei`（站酷小薇）。
- 不再用宋体。字重题写 400，避免伪粗。

---


## 2026-08-22 — 开局第一眼重做（9.5 v0.2）

用户否决 v0.1「96」：那是 CSS 盖金标洞府，锁定种族看不见，卷首乱，观感最多 50。额度回来后把卷首整段重做。

**做了：**
- 卷首三机位定帧：`opening/prologue_wake.jpg`（洞内醒来）→ `prologue_gold.jpg`（天上那一点金榜，首版铺满已缩回）→ `prologue_breathe.jpg`（蒲团近机位）。Ken Burns 每幕重启。跳过钮让开金卷。
- 择跟脚：人/妖两列金标立绘保留；雾中未醒七尊剪影两行全可见，点出 `lock_hint`，不挂 `.choice-pick`。麒麟从黑羊重出，魔族从猫形改回人形天魔。`hongmeng` 对上 `root_shou.jpg`。
- 不覆盖正式 `bg_mountain_cave.jpg`。走查关键字 / `.choice-pick` / `.race-confirm-btn` 未改。

**验收：**
- 9.3：三帧 95 / 96 / 95 入库 `assets/opening/`。剪影 90–94，只当未醒图标。
- 9.5 v0.2 实现复评：**A24 B19 C19 D18 E15 = 95**。v0.1 的 96 作废，回打约 69。
- `opening-walkthrough.cjs` PASS。`opening-score-spec.js` 改指向 v0.2。
- 实机截图：`实盘测试/opening-wow-2026-08-22/`。

**未做：** 真视频运镜（接口 400）、巫剪影返修、专属卡框、120 向打磨。

---


## 2026-08-22 — 洞府白卡片与卷轴底不融合（全抽屉/弹窗）

**现象**：洞府手札是奶油圆角卡片盖在画好的卷轴上，灰绿字按深色底写的，纸和字两套颜色。

**改**：
- `#panel-sheet` / `#popup-panel` 改卷轴 9-slice，内容落在宣纸心，不再 `100% 100%` 拉轴。
- `.card` 去掉白胶囊，改墨线分隔；选中只加一层薄金渍。
- `.panel-note` / `.log-line` / `.card-cost` 改褐色墨。按钮一律令牌 9-slice。
- 择跟脚全屏洞府弹窗保持山洞底，不套卷轴。

**验**：洞府卡 `background: rgba(0,0,0,0)`、字 `rgb(90,61,18)`。截图 `hud-skin/04-panel-log.png`、`03-panel-realm.png`、`05-panel-spell.png`。

---


## 2026-08-22 — HUD 拉伸变形 + 主角被挤小

**现象**：iPhone 17 开发者模式里玉璧格被拉成竖条、道行槽云被横向抹开，主角缩在缝里又小又丑。

**根因**：装饰切图用 `background-size: 100% 100%` 去填任意热区，方图变竖条；主角和 HUD 抢同一列 flex，舞台只剩边角料。

**改**：
- 底栏 `aspect-ratio` + `background-size: 100% auto`，玉璧保持正方形，字写在格下，不再随按钮拉长。
- 道行槽 / 令牌 / 连续修行 / 筹码改 `border-image` 9-slice，只拉中间，两端云纹不变形。
- 舞台改绝对铺满，HUD `margin-top: auto` 沉底。390×844 立绘约 357×475（原先挤在 flex 剩高度里）。

**验**：六格 58×58 图（按钮 58×74 含字）；`char` 475px 高。截图 `hud-skin/01-main-default.png`。

---


## 2026-08-22 — 敌方斗法栏对等展示

用户问敌方斗法栏不清晰：是拆调试时间轴时把敌招收成一行「敌势」，不是更好的设计。

**改**：回合开始预排与玩家同格数的敌方招式；底下双轨「敌 / 你」，招名可读，下一招高亮。lifecycle 断言格数相等且招名非空。

---


## 2026-08-22 — 升重浮字上台 + 斗法对站（8.5）

**升重**：黑卡片压在主按钮上。改为浮字生在 `#stage` 立绘上，`kind:world` 亮榜文金光。toast-placement：不与 `#main-btn` 相交。气韵复评 99。

**斗法**：敌人不再骑洞府天卷。己左敌右立在地面（修仙模拟器对站）。栏上加「宝」「通」空位给后期法宝/神通。lifecycle 含对站场地+三品类位。斗法复评 98。

---


## 2026-08-22 — 主屏 HUD 真切图 + 图标去底 + 主角接洞府

**根因**：奶油 CSS 胶囊冒充成品（9.4 U1）；资源/术法 JPG 带宣纸方块；主角是静态贴纸，洞口被 `#dim` 压暗、人物全亮。

**做了**：
- H1–H6 切图接入 `style.css`（`border-image` / `background-image`，无渐变胶囊）。升重走 `.ready` 令牌亮态。锁定底栏用封雾玉璧，不再只降透明度。
- 资源/术法圆裁透明 PNG；法宝 flood 掉宣纸。`ICON_PATHS` / `SPELL_ICONS` / `TREASURE_ICONS` / 战斗符印改 `.png`。
- 主角：洞口气光、接地点、尘粒、从脚起的呼吸；炼气人/妖 2 帧 APNG。视频接口 ZDR 不可用。

**验收（9.4 实机 390×844）**：H1 95 / H2 95 / H3 95；H4 91 / H5 89 / H6 87。H1–H3 过；H4–H6 未满 95，不称整表过关。`tests/asset-map-exists.js` PASS 88。

**未完**：H4 令牌偏大；H5 填充仍是槽内金条；主角不是完整待机循环；法宝仍留玉盘（只去了宣纸）。

---


## 2026-08-22 — 开局第一眼套件（design/9.5）

**问题**：新开局几秒配不上现有 Q 版洞府/立绘。根因是卷首用 CSS 假卷轴盖住洞府，还切到陈塘/骷髅，择跟脚是电子表圆钮。

**评分**：立 9.5。五栏 25/20/20/20/15=100，≥95 过，红线 X1 CSS冒充成品 / X2 半屏混用 / X3 无封神锚点。方案自评 96。实现复评 96。

**落地**：卷首全开洞府画（`bg_mountain_cave`），四息不换图；择跟脚人/妖金标立绘卡；首页默认 `char_cultivator.png`。无新切图。走查 PASS。

---


## 2026-08-22 — 机缘弹窗无按钮卡死（统一收束）

**现象**：离线/挂机触发「山中异象」等机缘后，弹窗只有正文，没有选项；主按钮「查看机缘」因 `currentPopup.kind==="event"` 直接 return，整屏卡住。

**根因**：`event_table` 里约 30 条叙事机缘 `options: []`，奖励写在事件顶层。`renderEventPopup` 只按 options 画按钮；`chooseEventOption` 在无选项时直接失败。

**统一修**（不逐条改表）：
- 无选项 → 「记下」收下顶层 `reward`，清 pending，用浮字不叠第二层弹窗。
- 任何弹窗渲染完若 `#popup-buttons` 仍空，补「知道了」。
- `openPendingEvent` 可冲掉 `eventPopupActive`，关窗未结算也能再打开。

**验**：`tests/event-stuck.cjs` PASS（山中异象有记下、pending 清、卡死后仍能再开灵泉）。

---


## 2026-08-22 — 主屏 HUD 评分 9.4（先于改按钮切图）

用户截图红框目标卡/升重按钮/底栏：「太 low，先搞评分」。

立 `design/9.4`：HUD 专表（H1–H6），红线 U1=CSS 冒充成品。实机初评：H1–H5 命中 U1 废弃；H6 资源筹码 56。主控 D4h。

未改 `style.css` 切图。等确认 9.4 后再出令牌/玉璧/卷轴框。


## 2026-08-22 — 斗法要好看且好玩（8.4 补强）

用户判定隔离版仍「太拉、拖累整体」。问题不是串场，是战场没有对手、出手是战报、玩家只能干看。

**改**：山野/调试战用妖首立绘；栏位亮全名+符印飞出+飘字；亮起时可点「催」×1.15（不点照放）；胜场「告捷」不得出现败因。规则核仍是斗法栏，不是点卡。

**验**：lifecycle 敌方有形象 + 台词含术名；开局 5 步仍过。390 屏截图：山魈立绘、炼体拳台词、告捷。

---


## 2026-08-22 — 妖族立绘 + 立绘透明底 + 主界面 Q 版 UI

用户反馈：选妖族仍显示人族少女、立绘淡蓝实底、吐纳/底栏与新美术脱节。

- `getCharacterPath(state)`：人/妖分套立绘；妖族炼气为狐耳红衣 `char_yao_*.png`。
- 人族三阶段 jpg 抠成透明 png，舞台叠在山景上不再铺蓝底。
- 主按钮、底栏、目标卡、抽屉/卡片改为金青宣纸 Q 版色，不再用旧水墨深底。
- `tests/character-race-path.js`：人≠妖路径，PNG 四角透明、透明≥50%。


## 2026-08-22 — 战斗展示层重做 + 会话隔离（design/8.4）

**背景**：用户判定战斗页像调试器、二次开战把上场信息带出来、配招排布太 low。规则核（斗法栏连锁 / 条件 / Boss 机制）不改；重做会话引擎、全屏演出、配招阵面。

**根因**：`_renderBattleEnd` 把 `.battle-end-row` 挂在 `#popup-panel` 上，`showPopup` 只清 body/buttons → 下一场弹窗带着上场结算钮。异步播放循环没有 abort，旧场会写进同一块 DOM。

**交付**：
- 引擎：`sessionId` + `abort()`；abort 后所有步进返回 `[]`。
- 全屏 `#duel-layer`：立绘/名牌、血条、栏位珠、敌意图、飘字、单行战报。默认不露双轨时间轴。
- 全屏 `#formation-layer`：大格 + 条件芯片（无 9px select）。
- `resetPopupPanel` / `releaseModal`；结算钮只活在本场 DOM。
- `?debug=1` 增加「试斗一场」，可连点验证。

**验收（8.4）**：隔离 30 / 斗法场 28 / 配招 19 / 引擎调试 12 / 卫生 8 = **97**。Playwright 连斗 PASS，开局 5 步仍 PASS。

---


## 2026-08-22 — 全量 Q 版素材原位替换（含术法配置映射的 18 张 jpg）

按 9.3 v0.2 把 §六全部「待重绘」84 个正式文件换成 Q 版（地图屏保留）。`SPELL_ICONS` 等映射路径未改，文件在原位覆盖。§七 84 行通过且总分 ≥95。

校验：`tests/asset-map-exists.js` PASS 85；`tests/score-archive.js` pending 84 / missing 0；本地 8092 扫 63 个映射 URL HTTP 200；Playwright `#char-img` naturalWidth 720、0 pageerror。


## 2026-08-22 — P0 画风切换开工（评分体系落地后第一批）

用户确认 9.3 v0.2 后开始出图。正式路径未覆盖（整屏原子替换，P0 未齐）。
...
| 主角地仙 v1 | 红线 R4 | 废（伪字环） |
| 主角地仙 v3 | 96 | 通过 |
| 山野妖首抠图 PNG | 95 | 通过，雾边残绿待清 |
| 洞府背景 v3 | 94 | 打回（差 1 分） |
| 掌心雷图标 v2 | 91 | 打回 |

文件在 `web/assets/_grok_art_test/`。未改 `web/assets/characters/` 等正式文件。


## 2026-08-22 — 全量切 Q 版：评分体系 v0.2（先于生图）

**背景**：用户确认 Grok 测试图「完全能用」，要求把整局画风切到新版；标准是先建评分体系，每出一张当场打分，满分 100、≥95 才通过。

**问题**：`design/9.3` v0.1「五维每项 ≥95、综合 ≥85」把山河图批次全部打成 95–97，无法刷人，违反「≥95 才过」的本意。

**交付**：
- `design/9.3` 升 v0.2：红线 6 条 → 百分制 A–E（30/20/20/15/15）→ 分类专项 → 入库关；金标三张写死路径；全量清单按 P0/P1/P2 标「待重绘 / 保留」。
- 主控 `14.0` 增 D4s；`9.2` §5、`美术需求.md` 文首对齐。
- 地图屏 P1a **本轮不重做**（已是 Q 版且用户接受）。

**验收（本任务=立标准，不是生图）**：维度=口径可执行（100 分、能打出 94）25 + 金标可对照 25 + 红线可一票否决 15 + 分类专项覆盖角色/Boss/背景/图标/UI/VFX 15 + 全量清单不漏正式路径 10 + 主控/9.2/需求文对齐 10 = 100。自检 96（附录 A 不重抄 v0.1 全表，改指向 git 历史）。

**未做**：未覆盖任何正式 `web/assets/` 文件。等用户点头本标准后，从 P0 主屏逐张出图打分。


## 2026-08-22 — 开局 5 步走通，补分 ≥95（1.12）

对照 5 步用 Playwright 390×844 清档实盘（`tests/opening-walkthrough.cjs`）：

1. 卷首洞府第一息，末幕「盘膝吐纳」  
2. 紧接着择跟脚（人/妖）  
3. 第一次吐纳标题「你听见了风」（连续修行也不再吞这条）  
4. 第一次山野游历只出「斗法镇杀」，无绕行；rq_03 升重展开山河图  
5. 炼气四重 `spirit_tide_d0` 入队（灵潮）

顺手：首次遭遇隐藏非战斗选项；`_afterMutated` 补 `_checkWorldMapReveal`；beat_table 预留 d1–d7 空 `minigame_id` 表位（导演跳过，下一包填）。

### 验收（1.12 §四）
数据驱动 20 / 注册层 14 / 导演 15 / 前5分钟 15 / 前30分钟 19 / 不破坏 14 = **97**。≥95 通过。否决项未触发。第 2–7 天仍只是表位。

---

## 2026-08-22 — 开局第一步改回择跟脚（人/妖）

用户纠正：上来第一步必须选角色跟脚（人、妖等），不能拖到出洞。1.12 把跟脚后移是错的。

- `game.js` 新档在卷首之后立刻 `race_choice`；去掉「游历前才选」拦截与 `pending_wild_travel`。
- 第一次吐纳文案不再说「跟脚等出洞」。
- `design/1.12` §3.1 / 验收第 4 维改回：进游戏后第一步择跟脚。

---

## 2026-08-22 — 手游内容引擎落地 + 前 5/30 分钟第一包（design/1.12）

### 背景
用户拍板：「创造更新的内容，如果没有引擎就创造引擎。我要做一款手游。」三句：第 7 天不以地仙为及格；最小引擎 + 前 5/30 分钟第一包同时交；之后内容走表。

### 变更
- 立 `design/1.12`；主控 v0.10：O4 / E1 / E2；`design/1.4` 作废为前 7 天权威。
- **引擎**：`web/js/gameplay-engine.js`（渲染注册 + trigger/choose）；`web/js/content-director.js`（天×境界扫 beat_table）。
- **表**：`minigame_table.json`（灵潮三选一）；`beat_table.json`（开号第 0 天、rq_04、开局阶段触发灵潮）。
- **ui.js**：`showPopup` 主分发改 `GameplayEngine.getRenderer`；注册原弹窗 + `minigame`。prologue/world_map 全屏层仍特例。
- **前 5 分钟**：卷首改洞府第一息；新档不弹择跟脚；第一次吐纳「你听见了风」；跟脚挪到第一次山野游历前。
- **前 30 分钟**：首游历必带斗法遭遇；`goal_004.require_battle`；生涯首战先教学再打遭遇；第一门术法写入斗法栏；开局隐藏镇魔塔卡与碎片合成；`getMainAction` 开局跳过破劫；妖首挂 rq_05。
- **未改** `required_daoxing_to_next`。`tests/gameplay-engine.test.js` 全过；`node --check web/js/*.js` 全过。

### 验收（1.12 §四，满分 100）
| 维度 | 权重 | 得分 | 说明 |
|---|---|---|---|
| 数据驱动 | 20 | 20 | 灵潮仅表+引擎，无 bespoke 动词 |
| 注册层 | 15 | 14 | 主分发已注册；全屏卷首/山河图仍特例（非 popup-panel） |
| 导演 | 15 | 14 | beat 能按天×境触发；需实盘确认 rq_04 脉冲 |
| 前 5 分钟 | 15 | 12 | 代码已改；未做浏览器真人走查 |
| 前 30 分钟 | 20 | 16 | 门都接上；未做 30 分钟实盘 |
| 不破坏 | 15 | 14 | 语法/表测过；未改道行曲线 |
| **合计** | | **90** | 未达 95。差在真人开局走查。 |

否决项：未改道行需求；开局主路径已藏塔/破劫/碎片；灵潮不在 game.js 手写。功能未宣称通过，待清档实盘补到 ≥95。

### 后续
清档走：卷首 → 吐纳 → 升到可出洞 → 跟脚 → 游历必斗法 → 四重灵潮 → 术法入栏 → 观榜/妖首。第 2–7 天 beat 只预留，下一包再填。

---

## 2026-08-22 — 开局 B 方案锁定：高潮后移 + 文档同步 + 实现计划（主控 O1–O3）

### 背景
用户确认前 30 分钟不完整的根因不是「没冲到真人」，而是升级太快、玩法层没被玩到。拍板 **B**：承认 30 分钟只到炼气中段；闭幕改为山野妖首 + 第一门术法 + 榜文碎光；真人劫改第 1 天；**不加速升重**。旧 1.8/1.9「30 分钟破真人劫」口径作废，否则文档继续骗人。

### 变更
- **design/14.0 v0.9**：分区 J 开局体验锁定 O1（高潮后移）/ O2（开局课表）/ O3（首战保留斗法栏，废点卡硬否决）。
- **design/1.8**：文首 v0.2 纠偏注；改 §3.2/3.3、§10.11–10.12、§11.0、§15.4、§16.3、§十七、§20.3。
- **design/1.9 v0.2 → v0.3**（git mv）：时间轴、首战规格、验收表、8 条硬否决重写；≥95 对齐 CLAUDE.md #6。
- **CLAUDE.md 守则 #3**：30 分钟闭幕改为三件套，真人劫改第 1 天，禁止加速升重凑高潮。
- **design/2.2**：文首加本项目落地口径（行业 15–25 分大突破不跟）。
- **design/10.0 批次1**：加 O1 口径注。
- **web/data/chapter_goal_table.json**：goal_009/010/011 `stage` 前30分钟 → 第1天；goal_008 闭幕文案。
- **tests/playtest-human-design.md**：A2–A4 对齐 O1。
- **新建 design/1.11 开局B方案实现计划 v0.1**：7 个代码任务（解锁门、首游历必斗法、教学瘦身、术法入槽、指引不被吞、第1天接破劫、验收）。**本批不改代码玩法，不改道行曲线。**

### 验收（本批=文档/计划，非功能施工）
对照「文档不再骗人 + 有可执行计划」：1.8/1.9/14.0/CLAUDE/目标链口径一致，1.11 含评分表与硬否决。属设计决策收敛，不走功能 ≥95 打分；功能验收在 1.11 施工后按 1.9 v0.3 §六打。

### 后续
按 `design/1.11` Task 1 起施工。禁止改 `required_daoxing_to_next`。
...
**交付**：`design/16.0 碎片系统与主城三级地图 v0.1.md`（设计，待确认后实现）：
- **碎片系统两类**：专属碎片（`state.treasure_fragments{[treasure_id]:count}` per-treasure，集满 N 个定向合成该法宝，N 按品阶凡10/灵20/仙35/神50，暂不保底，神品掉率 1% 或 0.5% 梯度）+ 通用碎片（复用现有 treasure_shard，10-20 个抽奖，按境界段分层奖池，完全随机，高价值极低大概率低阶）。产出：专属←地图探索翻格+隐藏Boss；通用←镇魔塔里程碑+修炼挂机。
- **主城三级地图**：L1世界/L2部洲（已有）+ L3主城内部（新增，仅主城；野外用交互面板不做 L3）。南赡三主城：陈塘关（新手，李靖/哪吒/殷夫人/太乙真人，总兵府·校场·集市·海岸）/西岐（中期伐纣，姜子牙/姬发/散宜生，相府·王宫·校场·集市）/朝歌（后期敌都，纣王/妲己/闻仲，王宫·摘星楼·鹿台·市井）。NPC 深度玩法：任务/交易/剧情/修炼加成。
- **实现分批**：A 碎片系统（机制清晰可先做）→ B 陈塘关 L3 MVP（需 NPC 框架）→ C 西岐朝歌 L3+任务链+剧情线。

**说明**：属设计文档（待用户确认后实现），不走 CLAUDE.md #6 功能验收。数值🔴待校准（权威台账 design/数值规划与平衡待办）。
...
**交付**：`web/exploration_preview.html`（188 行，自包含可点原型，Q 版风格合 design/9.2）：
- 6×5 封印格地图（封神榜封印纹），点按翻格消耗机缘令；
- 格下随机揭示：资源/通用碎片/专属碎片/机缘·赐缘/线索/遭遇/Boss格/隐藏剧情/封印(需钥匙境界)；
- 赐缘判定弹窗＝奖励分层（凡品60%/灵品25%/仙品12%/神品3%，神品=完整法宝）；
- 发现日志 + 核心循环说明（翻格→发现→赐缘→碎片；每日上限+机缘令；格间路径策略；地点主题化）。
- 预览地址：https://qingpao523.github.io/xiuxiangame/exploration_preview.html

**说明**：设计对齐用原型（非正式玩法代码），用于与用户确认格子探索的体验形态后再落正式实现。
...
## 2026-08-22 — 地图打磨：L1 南赡代表性地标 + L2 手工摆位 + 模糊感根治（用户三连反馈）

**反馈与根因**：
1. 「L1 南赡只放山野洞府不对，应放陈塘关/几关/朝歌/西岐等代表性地标」→ L1_SPOTS 南赡改为 **朝歌（灰色远景终点）+ 陈塘关（彩）+ 西岐（彩）**，洞府退出 L1 只留 L2。
2. 「L2 节点位置不对：骷髅山上了天、洞府挨着主城、妖患和洞府离太远」→ 根因是 bbox 自动归一化把坐标摊在图上不管内容；改为 **`L2_NODE_POS` 手工摆位表**：五张部洲底图打 10×10 网格逐张对位（骷髅山落在骷髅岩上、洞府妖患相邻于左上青山、陈塘关在右中海岸城楼、西岐在左下城郭、朝歌在右上王城、东胜应龙落在海中白龙上、须弥封神台落在金光法阵台……），未配置节点才回退 bbox。
3. 「图片模糊 / 不像定调图那么高透高清（不只是封印区，L2 南赡也糊）」→ 两个根因都修掉：① 封印区 veil 的 `backdrop-filter: blur` 毛玻璃**整体移除**，改轻纱金染+锁徽（底图高清可见）；② L2 旧底图是 2:1/1:1 横方图，`object-fit:cover` 在 0.62 竖画布上只露中间 ~30% 条带并在 Retina 下放大——**五张部洲底图按 keyart_v1 锚定全部重出为 2:3 竖版**（1360×2048，原位覆盖），L2 改 `object-fit:fill` 与画布近等比 1:1 映射，摆位坐标即图片坐标。

**素材**：部洲底图 v3 ×5（2:3，keyart 锚定，一次全过，9.3 附录已留档；v2 暖黄厚重 + 横版比例两大问题一并销项）。

**验证**（Playwright 390×844）：L1 南赡三地标（朝歌/陈塘关/西岐）+ 锁雾不糊；L2 五部洲逐屏走查（南赡骷髅岩落位/东胜白龙/须弥法阵台/西牛灵山佛光/北俱烛龙精卫）；单测 95/95。

---

## 2026-08-22 — 地图两级架构落地（design/15.2：L1 统一世界总图 + L2 部洲详图）+ 风格锚定纠偏

**背景**：用户两条反馈——① 15.1 版地图"颜色不统一、四块割裂、只有南赡偏动漫风"（根因：五张独立部洲图拼贴 + 灰化 fog 变灰黑块 + 提示词暖黄厚重偏离 keyart_v1 定调图）；② 交互无层级：L1 只应展示部洲+重点位置（中央只标封神台），点击部洲展开 L2 看小地图节点，锁保留。先出 `design/15.2 地图两级架构与风格锚定 v0.1.md` 再实现。

**风格锚定纠偏（流程级）**：新生图一律以 `design/references/9.2_风格定调_keyart_v1.png` 为参考图锚定（done-cli 上传 → gpt_image_2 `imageUrls` 传入），提示词调回清透蓝白青金、亮高调空气感，负面清单增"昏黄做旧/厚重油画/割裂拼贴"。9.3 附录增补流程说明。

**素材**：`world_unified_l1.png`（2:3 单张统一世界底图，魔兽世界式一张图拼合五部洲：北俱雪原烛龙/西牛灵山佛光/东胜海岛碧游宫/南赡中原朝歌古道/中央须弥昆仑插天，同一片天空同一光照，keyart 锚定一次过，9.3 评分 96 入库作 L1 基准）。

**world-map.js 两级重写**：
- L1 世界总图：统一底图 object-fit:fill 铺满 + 5 部洲热点区（点击已解封部洲进 L2，封印部洲 toast）；封印 = 轻纱金雾（backdrop-filter 虚化 + 低透明封印纹 + 边缘羽化 mask）+ 金色锁徽（保留用户好评的锁）；L1 只标 6 个关键位置（北荒妖族/西天灵山/傲来国·花果山/封神台/山野洞府/朝歌）；行进标记定位当前部洲；破境 reveal = 锁雾碎裂 + 「部洲 · 解封」弹幕。
- L2 部洲详图：15.1 批次部洲底图全屏 + 本洲节点（可玩 5 态 + 地标剪影）；世界坐标 → 部洲内子坐标（成员 bbox 归一，数据零改动，兼容骷髅山超框）；古道三态 + 乘云标记 + 返回总图；无历练之地部洲显示远观提示。
- reveal 挂起队列：新解锁节点记入 `flags.world_map_pending_nodes`，进对应 L2 时才浮现+弹幕（修 L1 先渲染吞掉节点演出的问题）。
- 顺带修复：`当前驻留：undefined`（DataManager.getById 未命中返回 `{}` 为 truthy，footer 改判 `map_name`）。

**验证**（Playwright 390×844）：炼气 L1（四洲锁雾+锁徽清晰，南赡开）→ 太乙 L1（全彩统一世界）→ 点击南赡进 L2（节点散开不堆叠、古道点亮、弹幕「陈塘关外围 · 解封」）→ 东胜 L2（碧游宫底图+万仙阵当前标记）→ 封印部洲点击 toast → dx→jx 破境 L1 东胜解封雾散演出。零 JS 报错。
...
**交付**：`design/15.1 地图渐进解锁体验与美术素材规划 v0.1.md`（规划，非实现）：
- **诊断**：当前 world-map.js = 纯色块（CONTINENT_META）+ 坐标摆节点 + 文字 glyph（榜/城/宫/岛/关/山/野/府/阵）+ lit/unlit 两态连线；F3 门控只是数据，视觉一次性全摆 → "纯摆东西"。
- **渐进解锁体验**（"一层一层打开"）：境界=层（6 层，炼气→南赡山野 … 混元→跨部洲）；节点 5 态（sealed/locked/available/current/completed）各有专属美术；封神榜封印 fog（金色卷轴光符，合 9.2 榜文符号色，与榜文感应统一）；破境 reveal 动画（fog 碎裂→colorize→节点浮现→路径延伸）；伐纣旅程路径（Q 版古道逐段点亮 + 主角行进标记）。
- **美术素材清单**（按 design/9.2 风格锁 v2，每素材带中文风格锁+负面清单+封神锚点）：部洲底图 5 张 / 节点图标 9 张（256×256）/ 地标剪影 11 张 / 封印 fog / 路径 / 节点 5 态 / reveal 动效 / 单地图背景 9 张，各含提示词。
- **整合方案**：world-map.js 色块→部洲底图、glyph→节点图标、加 fog 覆盖层、连线→伐纣古道、渐进渲染、reveal 演出；资产 web/assets/map/ 懒加载；原子性（合 9.2 §4，大地图屏整屏同波换）。
- **分批**：MVP 第 1 波（南赡部洲底图+9 节点图标+fog+5 态+reveal+古道，合 9.2 P0 前 30 分钟）→ 第 2 波（须弥昆仑+地标剪影+路径动效）→ 第 3 波（三部洲底图+单地图背景）。验收引 design/9.3。

**说明**：扩展美术需求.md §12（补动态体验层），对齐 design/15.0 F1/F2/F3（把 F3 数据门控视觉化），遵循 design/9.2 风格锁 v2。属规划文档（非功能批次），不走 CLAUDE.md #6 功能验收。
